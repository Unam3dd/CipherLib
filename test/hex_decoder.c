...
int main(int ac, char **av)
{
	...
	hex_decode((uint8_t *)av[1], decode, len, sizeof(decode));
	...
	if (strncmp((char *)to_lower(decode), (char *)decode_pipe, len >> 1))
		return (fprintf(stderr, "Error output is not the same !\n"), 1);

	printf("Hex Decoder : %s\n", decode);
	printf("Hex Decoder with pipe : %s\n", decode_pipe);
